...
W = helper.make_tensor_value_info('W', onnx.TensorProto.FLOAT, [1, hidden_size, input_size])
R = helper.make_tensor_value_info('R', onnx.TensorProto.FLOAT, [1, hidden_size, hidden_size])
B = helper.make_tensor_value_info('B', onnx.TensorProto.FLOAT, [1, 2 * hidden_size])
H0 = helper.make_tensor_value_info('H0', onnx.TensorProto.FLOAT, [1, batch_size, hidden_size])
Y = helper.make_tensor_value_info('Y', onnx.TensorProto.FLOAT, [sequence_length, 1, batch_size, hidden_size])
Y_h = helper.make_tensor_value_info('Y_h', onnx.TensorProto.FLOAT, [1, batch_size, hidden_size])
...
# Generate random initial weights and inputs
W_data = np.random.randn(1, hidden_size, input_size).astype(np.float32)
R_data = np.random.randn(1, hidden_size, hidden_size).astype(np.float32)
B_data = np.random.randn(1, 2 * hidden_size).astype(np.float32)
H0_data = np.random.randn(1, batch_size, hidden_size).astype(np.float32)
